...
class BLatStanley_controller(Controller):
    ...
    def run_in_series(self, next_waypoint: Transform, **kwargs) -> float: #*********** aka stanley_control(state, cx, cy, cyaw, last_target_idx)
        ...
        *** output lat_control:  steering angle delta = heading error + inv tan (gain * cross track error/veh speed)
        ...
        lat_control = float(
            np.clip((head_err + np.arctan(k * pos_err / (veh_spd + 1e-5))) / 60,
            self.steering_boundary[0], self.steering_boundary[1])
            #**** guessing steering of '1' equates to 60 degrees
        )
    ...
    def stan_calcs(self, next_waypoint: Transform, **kwargs):
        ...
        print('next waypoint (global) =', next_waypoint.location)
        ...
        vf_cte = self.calc_cross_track_error(next_waypoint) # vf_nextwp[0] # vf_npath1[0]
...
#*************************
        ...
        print('veh x = ', veh_x)
        print('veh z = ', veh_z)
        print('diff x =', np.abs(veh_x - next_waypoint.location.x))
        print('diff z =', np.abs(veh_z - next_waypoint.location.z))
